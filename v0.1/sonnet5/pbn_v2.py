...
def posterize_smooth(img_arr, method, blur_sigma):
    """
    Pre-smooth the image before color quantization (posterization).

    'meanshift' (default): edge-preserving mean-shift filtering, the classic
        technique behind cartoon/posterize effects. Flattens texture and
        noise within regions into solid flat color while keeping hard
        boundaries crisp.
    'bilateral': edge-preserving bilateral smoothing (3 passes for a strong
        enough effect). Similar goal to meanshift, slightly softer edges.
    'gaussian': simple blur. Softer still, and bleeds colors across edges.
    'none': quantize the raw pixels as-is (noisiest result).
    """
    if method == "none" or blur_sigma <= 0:
        return img_arr
    if method == "gaussian":
        return cv2.GaussianBlur(img_arr, (0, 0), sigmaX=blur_sigma)
    if method == "meanshift":
        sp = max(2, int(blur_sigma * 7))    # spatial window radius
        sr = max(20, int(blur_sigma * 40))  # color window radius
        return cv2.pyrMeanShiftFiltering(img_arr, sp=sp, sr=sr)
    # bilateral: needs a strong sigma + a few passes to meaningfully flatten
    # noise/texture, a single default-strength pass barely does anything.
    sigma = max(10, blur_sigma * 40)
    out = img_arr.copy()
    for _ in range(3):
        out = cv2.bilateralFilter(out, d=9, sigmaColor=sigma, sigmaSpace=sigma)
    return out


def quantize_colors(img_arr, n_colors, blur_sigma, smooth_method):
    """
    ...
    smoothed = posterize_smooth(img_arr, smooth_method, blur_sigma)
...
def main():
    ...
    parser.add_argument("--blur", type=float, default=1.5, help="Smoothing strength applied before posterization/quantization. Default 1.5.")
    parser.add_argument("--smooth-method", choices=["meanshift", "bilateral", "gaussian", "none"], default="meanshift",
                         help="Pre-quantization smoothing method. 'meanshift' (default) preserves edges for cleaner posterized regions.")
    ...
    print(f"Posterizing to at most {args.colors} colors ({args.smooth_method} smoothing) ...")
    label_map, centers = quantize_colors(img_arr, args.colors, args.blur, args.smooth_method)
